...
class TransferSelectSinpeMobileAccount extends StatefulWidget {
  final Account sourceAccount;
  const TransferSelectSinpeMobileAccount({
    super.key,
    required this.sourceAccount,
  });

  @override
  State<TransferSelectSinpeMobileAccount> createState() =>
      _TransferSelectSinpeMobileAccountState();
}

class _TransferSelectSinpeMobileAccountState
    extends State<TransferSelectSinpeMobileAccount>
    with UseStateMixin {
  ...
  bool get _isAccountNumberValid {
    return _accountNumberController.text.length == 8;
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<FavoriteAccountBloc>().add(
        FavoriteAccountsRequested(user: user, type: PaymentMethod.sinpeMovil),
      );
    });
  ...
  Widget build(BuildContext context) {
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<SinpeMovilBloc, SinpeMovilState>(
          listener: (context, state) {
            if (state is SinpeMovilError) {
              SweetAlert.show(
                type: SweetAlertType.error,
                message: state.message,
                context: context,
            ...
            } else if (state is SinpeMovilSearchSuccess) {
              final user = context.read<AuthBloc>().user!;
              final account = FavoriteAccount(
                alias: state.sinpeMovil.name,
                accountNumber: state.sinpeMovil.accountNumber,
                phone: state.sinpeMovil.phone,
                userId: user.id!,
              );
  ...
  Widget _buildBottomAction() {
    final bloc = context.read<SinpeMovilBloc>();
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
      child: IntrinsicHeight(
        child: PrimaryButton(
          ...
          onPressed: () {
            ...
            bloc.add(SearchSinpeMovilRequested(_accountNumberController.text));
          },
        ),
      ),
    );
